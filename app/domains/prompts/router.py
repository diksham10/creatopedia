...
async def create_prompt(
    ...
    content_type: ContentType = Form(ContentType.text),
    ...
    logger.info(f"[PROMPT CREATE] type={content_type} content={'set' if content else 'missing'} pdf_url={'set' if pdf_url else 'missing'}")

    # Smart content resolution
    if content_type == ContentType.pdf:
        # For PDF prompts, content holds the PDF URL.
        # The frontend may send it as `content` or as `pdf_url` — accept both.
        resolved_content = content or pdf_url
        if not resolved_content:
            raise HTTPException(
                status_code=400,
                detail="Please upload a PDF file before submitting. No PDF URL was received."
            )
        content = resolved_content
    else:
        # For text prompts, content must not be empty
        if not content:
            raise HTTPException(
                status_code=400,
                detail="Prompt content is required for text prompts."
            )
    ...
    try:
        ...
        logger.info(f"[PROMPT CREATE] ✓ Created prompt id={prompt.id} slug={prompt.slug} type={content_type}")
...
async def update_prompt(
    ...
    update_data = {}
    print(f"Received update for prompt {prompt_id} with fields: title={title}, description={description}, price={price}, ai_tool={ai_tool}, status={status}, gate_type={gate_type}, category_id={category_id}, content={content}, content_type={content_type}, thumbnail_url={thumbnail_url}")
    ...
    if content_type: update_data["content_type"] = content_type
